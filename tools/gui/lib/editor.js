...
(function (root) {
  ...
  const PANEL_CAP_MARKS = {
    ...
    /* 0.7.0 起：改正文的手势是**三击**而不是长按。单独一个标记是必须的——
       0.6.0 的面板能改正文、但手势是长按，而「应用到面板脚本」只换配置与分组、不换代码；
       没有这个标记，那种面板不会被判成旧版，用户就没有"换成新版"的路（真踩过）。 */
    tripleClick: 'FANO_PANEL_CAP_TRIPLE_CLICK',
    /* 0.6.0 起：顶部按钮=整块隐藏 / 壁纸开关 + 纯色底 / 小方案改名 */
  ...
  function exportChecks(json, edit, model, opts = {}) {
    ...
    if (panels.length) {
      ...
      const CAP_TEXT = {
        ...
        tripleClick: {
          kind: '面板是旧版：改正文的手势还是长按',
          what: '**三击改正文**（这份面板里还是长按；配置里写三击也不会生效，因为代码没换）',
        },
        controlsV06: {
