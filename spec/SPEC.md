...
## 5c. 面板 0.6.0：按钮改隐藏 / 壁纸开关 + 纯色 / 小方案长按改名
...
### 5c-3 小方案：长按 chip 就地改名

- 长按方案 chip（`CONFIG.edit.longPress.ms`）→ chip 就地变成输入框；Enter / 失焦保存，Esc 放弃。
- **不受 `CONFIG.edit.longPress.enabled` 影响**（那个开关管的是"长按条目改正文"，与小方案无关），
  所以 `bindLongPress` 加了 `{ force, onLongPress }` 两个选项复用同一条手势实现。
- 不用 `prompt()`：面板跑在酒馆页面里（可能是嵌套 iframe），原生弹窗在手机与沙箱里表现不一；
  就地输入框没这个问题，而且看得见自己在改哪一个。
- 点一下仍是"应用这个方案"、右键仍是删除。长按之后紧跟的那次 click 必须由 `onRowClick` **吞掉**——
  否则改完名会顺手把方案应用掉（与条目行是同一个坑）。
...
`tools/test-panel.mjs` 加 **25 项**（155 → 180）：按钮隐藏/回来/落盘、长按 chip 出输入框并改名、
